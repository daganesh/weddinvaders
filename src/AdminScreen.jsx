...
export default function AdminScreen({ onExit }) {
  ...
  const [seedId, setSeedId] = useState(DEFAULT_PACKAGE_ID);
  ...
  const isSystem = draft.isDefault;
  const systemPackages = packages.filter(p => p.isDefault);
  ...
  function handleCreatePackage() {
    try {
      const pkg = createPackage(nameInput, seedId);
      setNameInput('');
      refreshPackages();
    ...
    <div className="admin-screen">
      ...
      <section className="admin-package-bar">
        ...
        <div className="admin-package-new">
          ...
          />
          <select value={seedId} onChange={e => setSeedId(e.target.value)} title="Start from">
            {systemPackages.map(p => (
              <option key={p.id} value={p.id}>Start from {p.name}</option>
            ))}
          </select>
        ...
        {isSystem ? (
          <>
            ...
            <p className="admin-hint">
              "{draft.name}" is a system package and is read-only. Create a new package (seeded from
              it, via the dropdown above) to customize it.
            </p>
          </>
        ) : (
      ...
      <fieldset disabled={isSystem} className="admin-fieldset">
        <section>
          <h2>Portraits</h2>

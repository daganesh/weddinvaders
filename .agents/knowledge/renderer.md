...
| `drawTitle(ctx, cfg)` | 632 | `title` phase screen — draws only the starry background (via `drawFullScreenStarryBackground`) and the still-canvas-drawn side panel (`drawPanel`, empty state), no text. Practically unreachable in normal play: `Game.jsx` no longer shows a canvas `title` phase at all — the invitation screen (`InviteScreen.jsx`, a separate pre-game DOM screen, not canvas-drawn) is shown first, and its "Start Playing" button dispatches `START` immediately, skipping straight to `modeSelect` before the canvas is ever mounted. Left in place as the dispatch table's default/safety case rather than removed. See `game-jsx.md`'s "Invitation screen / game phases" and `architecture.md`'s "Opening-page links". |

...
| `drawAmmoHint(ctx, timer)` | ~530 | A bordered banner over the top of the play field explaining cash-vs-envelope ammo, drawn only while `state.ammoHintTimer > 0`. Non-blocking — gameplay keeps running underneath it, unlike the true overlay screens (`meeting`/`levelComplete`/etc.). Fades out over its last `FADE_FRAMES` (60) rather than disappearing abruptly. `ammoHintTimer` is only ever non-zero the first time a guest reaches Level 1 in a given browser — see `use-game-state.md`'s `shouldShowAmmoHint()`. |

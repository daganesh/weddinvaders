...
| `drawHUD(ctx, state)` | 428 | Top HUD: money, ammo count, lives, timer. `ammo.envelope` is the one ammo readout now (was two, `ammo.invite`/`ammo.heart`). The two **remaining-ammo** numbers — money (top-right) and envelope count (below the hearts) — are the largest, most prominent text in the HUD (`bold 21px`/`19px monospace`) and both turn red under a low threshold (money `< $200`, envelope `≤ 2`) — feedback was that it wasn't clear how much of either was left, especially once shooting silently stops at zero of both. The smaller `bold 13px` labels next to the envelope count (👰/🤵 + ammo emoji) are deliberately less prominent — they show *which* ammo is currently selected, not a quantity. When `state.livesFlashTimer > 0` (set by `useGameState.js` on a mine hit or a level failure — **not** an escaped item, which no longer costs a life), draws a fading red rect behind the heart icons before the rest of the HUD text, so losing a life reads clearly even off to the side of the player's focus — see `use-game-state.md`'s "Life-loss feedback". Update this when adding a new ammo type. Not customization-aware (out of scope — HUD/ammo labels aren't in the customizable text set). |
...
| `drawAmmoHint(ctx, timer)` | ~530 | A bordered banner, vertically centered in the play field (not pinned to the top edge — an earlier version was, and was reported as "not visible well"), explaining cash-vs-envelope ammo, drawn only while `state.ammoHintTimer > 0`. Non-blocking — gameplay keeps running underneath it, unlike the true overlay screens (`meeting`/`levelComplete`/etc.). Shows one of two short tips depending on how much of `timer` remains relative to `AMMO_HINT_PHASE_FRAMES` (`constants.js`) — cash while `timer > AMMO_HINT_PHASE_FRAMES`, envelope for the second half — fading out at the end of each phase (`FADE_FRAMES`, 30) rather than disappearing abruptly, including the handoff between the two tips. `ammoHintTimer` is only ever non-zero the first time a guest reaches Level 1 in a given browser — see `use-game-state.md`'s `shouldShowAmmoHint()`. |
| `drawMeeting(ctx, state, assets, cfg)` | 510 | `meeting` phase: couple image + level result. Uses `cfg.text.winMessage` on the last level, `getLevelText(cfg, ...)` for the next level's name/subtitle otherwise. Its two `drawPlayer` calls always pass `waiting=false` and each role's *natural* `isGroom`/`isTop` pairing (both sprites always shown side by side in their default orientation — the meeting cutscene is a fixed couple moment, not a snapshot of mid-game slot assignment; today this matches gameplay exactly, since `topRole`/`bottomRole` no longer vary by `soloRole` either). When there's a next level (not the final one), also draws a "+$X & +N 💌 waiting for you!" line in the HUD strip below the play field, reading `LEVELS[currentLevel+1].money`/`.envelopes` directly — the same values `getInitialState`'s `carryOverMoney`/`carryOverEnvelopes` will add to on arrival, see `use-game-state.md`. |

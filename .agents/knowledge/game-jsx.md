...
| `uiRequiredDone` state | Same pattern as the mirrors above (own `prevRequiredDoneRef`), but recomputed from `isLevelComplete(LEVELS[state.currentLevel], state.acquiredItems)` (imported from `./levels`) every frame inside the render callback rather than mirroring a state field directly — there's no `state.requiredDone` to mirror, this *is* the same check the keyboard "N" handler and `useGameState.js`'s row-advance speedup both use. Drives the mobile "⏩ Skip" button in `.solo-controls` (see below), the touch equivalent of "N" for a device with no keyboard. |
| render-callback effect | Registers a callback that calls `render(ctx, state, assets, config)` (see [`renderer.md`](renderer.md)) every frame, updates the `uiPhase`/`uiMode`/`uiSoloRole` mirrors on phase change, and updates `uiRequiredDone` when it flips. Registered once (deps don't include `active`) — starting/stopping the loop itself is a separate concern, see the next row. |
...
| `handleCanvasClick` | Maps a canvas click to the right `handleAction` for the current phase: `playing` in solo mode → `SHOOT` for `soloRole` (a couple-mode click does nothing — shooting stays on dedicated keys/buttons since both roles share the canvas); `meeting`/`levelComplete` → `NEXT_LEVEL`; `gameComplete`/`lost` → `RESTART`; `levelFailed` → `RETRY_LEVEL` (retries the same level, not a full restart — see `use-game-state.md`'s "Level failure & retry"). No `title` branch — by the time the canvas is ever clickable, the `START`-dispatch effect has already skipped past that phase. |
...
| `.mode-select-overlay` | Shown only when `uiPhase === 'modeSelect'` — a `.mode-select-rules` paragraph (the one-time "explanation and rules" a guest sees on first reaching the game screen, sitting in the gap below the canvas-drawn heading) followed by up to three DOM buttons (Couple / Solo as Bride / Solo as Groom) dispatching `handleAction({ type: 'CHOOSE_MODE', mode, soloRole })`. The Couple button is conditionally omitted (`!IS_MOBILE_LAYOUT`, from `constants.js`) below the 840px touch-controls breakpoint — see `game-design.md`'s "Game Modes" for why. Canvas-drawn backdrop/heading is `drawModeSelect` in `renderer.js`; the rules text and buttons themselves are DOM, not canvas, since they need normal text reflow and click handling. |
| `.mobile-controls` block | Inside `.game-frame`. Couple-mode-only (`showControls && uiMode === 'couple'`) on-screen ammo-select and shoot buttons for touch devices, dispatching `SELECT_AMMO` / `SHOOT` actions per role. `.ammo-btn`'s font-size was bumped (22px → 26px) so the ammo legend markers read more clearly. |
| `.solo-controls` block | Inside `.game-frame`. Solo-mode-only (`showControls && uiMode === 'solo'`) mobile control surface: a `.solo-controls-row` holding the "🔄 Switch Ammo" button (dispatches `CYCLE_AMMO` for `uiSoloRole`) and, only when `uiRequiredDone`, a "⏩ Skip" button (dispatches `handleAction({type:'SKIP_ADVANCE'})`, `useGameState.js`'s mobile-reachable equivalent of the desktop "N" key) — then the hint line ("Swipe canvas to move · Tap to shoot") *below* that row, not above it, so the interactive buttons sit closer to the canvas/player sprite for a faster reaction. Deliberately simpler than the couple-mode ammo-select row, per the mobile-first goal of solo mode; couple mode has no equivalent Skip button since there's no single "the Ammo button" there to anchor it next to (each player has its own ammo-select group). |
...
| `.end-of-level-banner` | A sibling **after** `.game-frame` (not inside it — see that row above). Shown when `uiPhase` is `meeting`, `levelComplete`, `gameComplete`, `lost`, or `levelFailed` (`END_OF_LEVEL_PHASES`, a module-level `Set`) — a plain block, not overlaid on the canvas, holding a "💌 Back to Invite & RSVP" link (`<a className="back-to-invite-btn" href="#/">` — plain hash nav now, not a callback prop). Appears on every level boundary, not just the final win, since clearing all 5 levels takes several wins and most runs end earlier. |

## Relationships / Cross-links
- Constructs and is the sole caller of `useGameState()` — see [`use-game-state.md`](use-game-state.md).
- Imports `LEVELS`/`isLevelComplete` from `src/levels.js` directly (not just through `useGameState()`)
  to compute `uiRequiredDone` — the same check `useGameState.js` uses internally for the row-advance
  speedup and the "N" key.

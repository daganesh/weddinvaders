...
| `.game-header-bar` | in JSX | The **only** thing rendered above the invite card / game frame, on both phases: the banner and the hamburger menu, side by side. No separate toolbar row, no persistent "Back to Invite" bar — see `.banner-link` and `.game-menu-anchor` below. `position: sticky; top: 0` in `Game.css` (with a solid background matching the body) — stays visible and reachable while scrolling, without needing to scroll back up, on either screen (matters most on the invitation screen, whose content can run taller than the viewport). |
...
| `.game-menu-anchor` / `.hamburger-btn` / `.game-menu-dropdown` | in JSX | A "☰" button toggling `menuOpen`; when open, a `.game-menu-backdrop` (full-viewport, click-to-close — same pattern as `AdminScreen.css`'s `.admin-modal-overlay`) sits behind a small dropdown with four items, in order: "🏠 Invitation" (calls `handleBackToInvite`), "🎮 Game" (calls `handleStartPlaying` — same guarded call the invitation screen's CTA uses, so picking this while a run is already in progress resumes it rather than resetting), "⚙ Admin" (navigates `window.location.hash = '#/admin'`, replacing the old standalone gear button), and "ℹ️ About" (sets `showAbout` true). Both nav items are shown unconditionally on both phases — picking "Invitation" while already there, or "Game" while already playing, is a harmless no-op. Rendered on both phases, top-right, next to the banner — the only settings/navigation entry point now; there's no separate `.game-toolbar` row any more. |

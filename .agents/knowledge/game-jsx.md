...
| `.game-wrapper` inline style | in JSX | Sets three CSS custom properties from `activeConfig.colors` — `--wv-bride-color`, `--wv-groom-color`, `--wv-accent-color` — read by `Game.css`'s `.bride-shoot`/`.groom-shoot` mobile shoot-button gradients and, since this feature, `.title-heading`/`.title-tagline`/`.title-prompt`'s glow/text color, so the title notice's accent matches whatever palette the active package sets instead of a hardcoded gold. |
...
| `linksRow` variable | in JSX | A `visibleLinks.length > 0 && (...)` expression (computed once per render, not a component) building a `.title-links` row of real `<a target="_blank" rel="noopener noreferrer">` pill links, one per `activeConfig.links` entry whose `url` is non-empty (`visibleLinks`, computed the same non-ref way as `activeConfig` above) — see `architecture.md`'s "Opening-page links". Both system packages ship with all three seeded links pointing at bundled pages, so this is visible out of the box, not just after an admin fills something in. Each `href` goes through the module-level `withProtocol()` helper: it prepends `https://` when the admin typed a bare domain (`example.com/rsvp`), but leaves a site-relative path (starting with `/`, `./`, or `../` — e.g. the bundled examples' urls) untouched, since prepending a scheme to those would break them. Real anchors, not canvas-drawn — clicking one lands on the `<a>` itself, never reaches `handleCanvasClick`, and `target="_blank"` guarantees it opens in a new tab instead of navigating the game away; verified via Playwright. Rendered in **two** places (see the next two rows), not duplicated — the same `linksRow` value is referenced both times. |
| `.title-overlay` | in JSX | Shown only when `uiPhase === 'title'` — the title screen's whole "notice," now a DOM overlay layered over the canvas (`position: absolute; inset: 0`, `pointer-events: none` on the container, re-enabled only on `.title-link` children — the same passthrough pattern as `.mode-select-overlay`, so a click anywhere else still reaches the canvas and starts the game). In order: `<h1 className="title-heading">` (`activeConfig.text.title`), `<p className="title-tagline">`, a `<p className="title-invitation">` with `activeConfig.text.invitation` (skipped entirely when blank — an admin can clear it), `linksRow`, a `.title-instructions` block (the bride/groom control lines and ammo tips — plain hardcoded JSX, not customization-aware, same scope boundary as before this notice became a DOM overlay), and a `.title-prompt` ("— Press any key to begin —"). Canvas-drawn backdrop is `drawTitle` in `renderer.js` (background + side panel only, no text) — see `renderer.md`. `line-height: normal` in `Game.css` overrides `.canvas-wrapper`'s `line-height: 0` (an inherited property that otherwise collapses/overlaps every line of text here). |
| persistent links footer | in JSX | `{showControls && linksRow}` placed as a normal-flow sibling right after `.canvas-wrapper` — the same `linksRow` value as inside `.title-overlay`, so the RSVP/registry/song-request (and any custom) links stay reachable below the canvas once the title/mode-select screens are dismissed, not just during the title phase. `showControls` (`uiPhase !== 'title' && uiPhase !== 'modeSelect'`) is the pre-existing "gameplay is active" flag reused as-is. `flex-shrink: 0` in `Game.css` on `.title-links`, same reasoning as `.game-banner`. |
